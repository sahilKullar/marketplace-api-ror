...
class Api::V1::ProductsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should update product" do
    patch api_v1_product_path(@product), params: { product: { title: @product.title,
                                                              price: @product.price,
  ...
  test "should forbid destroy product" do
    assert_no_difference("Product.count") do
      delete api_v1_product_path(@product), headers: { Authorization: JsonWebToken.encode(user_id: users(:two).id) },
                                            as: :json
    end
    assert_response :forbidden
  end
